<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="bg-white px-5 py-2.5 rounded-2xl border border-slate-100 shadow-sm text-left md:text-right self-start md:self-auto">
    ...
    <div class="py-6 sm:py-12 px-4 sm:px-6 lg:px-8 min-h-screen" x-data="{ activeLab: 'all' }">
        <div class="max-w-7xl mx-auto space-y-8 sm:space-y-10">

            {{-- HUD Statistics --}}
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                <div class="bg-slate-800 rounded-3xl sm:rounded-[2rem] p-6 shadow-xl relative overflow-hidden group border border-white/5">
                    <div class="relative z-10">
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Total Terminals</p>
                        <h3 class="text-3xl sm:text-4xl font-black text-white italic">{{ $totalComputers ?? 0 }}</h3>
                    </div>
                </div>

                <div class="bg-white rounded-3xl sm:rounded-[2rem] p-6 shadow-sm border border-slate-200 relative overflow-hidden">
                    <div class="relative z-10">
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Active Sessions</p>
                        <h3 class="text-3xl sm:text-4xl font-black text-slate-800 italic">{{ $activeStations ?? 0 }}</h3>
                    </div>
                </div>

                <div class="bg-rose-500 rounded-3xl sm:rounded-[2rem] p-6 shadow-xl shadow-rose-500/10 relative overflow-hidden sm:col-span-2 lg:col-span-1">
                    <div class="relative z-10">
                        <p class="text-[10px] font-black text-white/80 uppercase tracking-[0.2em] mb-1">Alerts Detected</p>
                        <h3 class="text-3xl sm:text-4xl font-black text-white italic">{{ $alertsToday ?? 0 }}</h3>
                    </div>
                </div>
            </div>

            {{-- Lab Filter --}}
            <div class="flex flex-wrap items-center gap-2 sm:gap-3 bg-slate-200/50 p-2 rounded-2xl sm:rounded-3xl w-full sm:w-fit overflow-x-auto">
                <button @click="activeLab = 'all'"
                    ...
                    class="px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all duration-200 whitespace-nowrap">
                    All Nodes
                </button>
                @foreach($labs as $lab)
            ...
            {{-- Topology Map --}}
            <div class="bg-white rounded-3xl sm:rounded-[3rem] p-6 sm:p-10 border border-slate-200 shadow-sm relative overflow-visible">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 pb-6 border-b border-slate-100 gap-4">
                    <h4 class="text-xl sm:text-2xl font-black text-slate-800 tracking-tighter uppercase">Terminal <span class="text-[#D4AF37]">Topology</span></h4>
                    ...
                    <div class="flex flex-wrap items-center gap-4 sm:gap-6">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <span class="text-[9px] font-black text-slate-400 uppercase">Maintenance</span>
                        </div>
                    </div>
                </div>

                {{-- Grid Container --}}
                <div class="grid grid-cols-3 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-12 gap-3 sm:gap-4 relative overflow-visible">
                    ...
                    <div
                        ...
                        @if($pc->status === 'active')
                        <svg class="size-3.5 mt-0.5 opacity-80 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                        @endif

                        {{-- Tooltip --}}
                        <div class="absolute bottom-[115%] left-1/2 -translate-x-1/2 w-48 sm:w-52 p-4 bg-slate-900 rounded-2xl 
                            ...
                            <div class="flex justify-between items-start mb-2">
                                <span class="text-[8px] font-black text-[#D4AF37] uppercase tracking-widest truncate max-w-[110px]">{{ $pc->lab->name ?? 'Unknown Lab' }}</span>
                            ...
                            <div class="text-[8px] text-slate-400 mt-0.5 uppercase tracking-tighter">
                                Since {{ optional($pc->activeSession->time_in)->format('h:i A') ?? 'N/A' }}
                            </div>

                            <div class="mt-3 pt-3 border-t border-white/10">
                                <form method="POST" action="{{ route('dashboard.sessions.terminate', $pc->activeSession->id) }}"
                                    onsubmit="return confirm('Terminate session for {{ $pc->activeSession->student_name }}?')">
                                    @csrf @method('PATCH')
                    ...
                    <div class="col-span-full py-20 text-center">
                        <p class="text-slate-300 font-black uppercase tracking-widest text-xs">No terminal nodes found in directory</p>
                    </div>
                    @endforelse
                </div>
            </div>
