<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <div class="flex items-center space-x-2 mt-1">
                    ...
                    <p class="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-[0.2em] sm:tracking-[0.3em]">
    ...
    <div class="py-6 sm:py-10 min-h-screen" x-data="{ 
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4 mb-6 sm:mb-8">
                <div class="flex flex-col sm:flex-row flex-1 items-stretch sm:items-center gap-3 w-full">
                    ...
                    <div class="relative w-full sm:w-72 md:w-80">
                        ...
                        <input type="text"
                            ...
                            class="block w-full pl-10 pr-4 py-3 sm:py-2.5 border-none rounded-2xl sm:rounded-xl bg-white shadow-sm focus:ring-2 focus:ring-[#D4AF37] text-sm transition-all">
                    </div>

                    <!-- Role Filter Dropdown -->
                    <select x-model="selectedRole" class="py-3 sm:py-2.5 px-4 border-none rounded-2xl sm:rounded-xl bg-white shadow-sm focus:ring-2 focus:ring-[#D4AF37] text-sm font-semibold text-slate-600 transition-all cursor-pointer w-full sm:w-auto">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                    <!-- Mass Enroll Button -->
                    <button @click="massEnrollModal = true" class="w-full sm:w-auto bg-slate-800 hover:bg-slate-900 text-white px-5 py-3 sm:py-2.5 rounded-2xl sm:rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 text-sm">
                    ...
                    <!-- Add User Button -->
                    <button @click="addModal = true" class="w-full sm:w-auto bg-[#D4AF37] hover:bg-[#b8962d] text-white px-5 py-3 sm:py-2.5 rounded-2xl sm:rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 text-sm">
            ...
            <!-- Main Data Container -->
            <div class="bg-white rounded-2xl sm:rounded-[2.5rem] p-4 sm:p-6 md:p-10 shadow-sm border border-slate-100 overflow-hidden">
                <div class="flex items-center justify-between mb-6 sm:mb-8">
                    <div class="flex items-center gap-3">
                        <h3 class="text-lg sm:text-xl font-black text-slate-800 tracking-tight">All System Users</h3>
                ...
                <div class="hidden md:block w-full overflow-x-auto">
                    <table class="w-full text-left border-separate border-spacing-y-2 min-w-[700px]">
                        <thead>
                            <tr class="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] bg-slate-50/50 rounded-xl">
                                <th class="py-3 px-4 rounded-l-xl">User</th>
                                <th class="py-3 px-4">Contact Info</th>
                                <th class="py-3 px-4">System Role</th>
                                <th class="py-3 px-4 text-right pr-6 rounded-r-xl">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="space-y-2">
                            @foreach($users as $user)
                            <tr class="group bg-white hover:bg-slate-50/80 rounded-2xl transition-all border border-slate-100/80 shadow-sm"
                                ...
                                <td class="py-4 px-4 rounded-l-2xl">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-xl bg-slate-900 text-[#D4AF37] font-black text-xs flex items-center justify-center shrink-0 shadow-sm border border-slate-800">
                                            {{ strtoupper(substr($user->name, 0, 1)) }}
                                        </div>
                                        <div class="flex flex-col">
                                            <span class="text-sm font-black text-slate-800 tracking-tight leading-snug">{{ $user->name }}</span>
                                            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{{ $user->student_number ?? 'N/A' }}</span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="py-4 px-4">
                                    <div class="flex flex-col">
                                        <span class="text-xs font-semibold text-slate-600">{{ $user->email }}</span>
                                        <span class="text-[11px] font-medium text-slate-400 mt-0.5">{{ $user->phone ?? 'No phone' }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="py-4 px-4">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider
                                        ...
                                        {{ $user->role == 'student' ? 'bg-amber-50 text-amber-600 border border-amber-200' : '' }}
                                        {{ $user->role == 'personnel' ? 'bg-slate-100 text-slate-700 border border-slate-200' : '' }}">
                                        <span class="size-1.5 rounded-full mr-1.5
                                            {{ $user->role == 'admin' ? 'bg-rose-500' : '' }}
                                            {{ $user->role == 'student' ? 'bg-amber-500' : '' }}
                                            {{ $user->role == 'personnel' ? 'bg-slate-500' : '' }}"></span>
                                        {{ $user->role }}
                                    </span>
                                </td>

                                <!-- Actions Column -->
                                <td class="py-4 px-4 text-right pr-6 rounded-r-2xl">
                                    <div x-data="{ dropdown: false }" class="relative inline-block text-left">
                                        <button @click="dropdown = !dropdown" class="p-2 text-slate-400 hover:text-slate-800 hover:bg-slate-100 rounded-xl transition-all">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div x-show="dropdown"
                                            ...
                                            {{-- Super Admin Delete Route --}}
                                            <form method="POST" action="{{ route('super-admin.users.destroy', $user->id) }}" onsubmit="return confirm('Permanently remove this user?')">
                                                @csrf @method('DELETE')
                ...
                <div class="grid grid-cols-1 gap-4 md:hidden">
                    @foreach($users as $user)
                    <div class="bg-slate-50/70 rounded-2xl p-4 border border-slate-100 space-y-4"
                        x-show="(search === '' || $el.innerText.toLowerCase().includes(search.toLowerCase())) && (selectedRole === '' || '{{ $user->role }}' === selectedRole)"
                        x-transition>
                        ...
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider shrink-0
                                {{ $user->role == 'admin' ? 'bg-rose-50 text-rose-600 border border-rose-200' : '' }}
                                {{ $user->role == 'student' ? 'bg-amber-50 text-amber-600 border border-amber-200' : '' }}
                                {{ $user->role == 'personnel' ? 'bg-slate-100 text-slate-700 border border-slate-200' : '' }}">
                                {{ $user->role }}
                        ...
                        <div class="text-xs space-y-1 bg-white p-3 rounded-xl border border-slate-100">
                            <div class="text-slate-600 font-semibold truncate">{{ $user->email }}</div>
                            <div class="text-slate-400 text-[11px] font-medium">{{ $user->phone ?? 'No phone' }}</div>
                        </div>

                        <div class="flex items-center justify-end gap-2 pt-1 border-t border-slate-200/50">
                            <button @click="editModal = true; currentUser = {{ $user }}"
                                class="px-4 py-2 bg-white text-slate-700 border border-slate-200 rounded-xl text-xs font-bold shadow-sm active:scale-95 transition-transform">
                                Edit
                            </button>
                            {{-- Super Admin Delete Route (Mobile) --}}
                            <form method="POST" action="{{ route('super-admin.users.destroy', $user->id) }}" onsubmit="return confirm('Permanently remove this user?')">
                                @csrf @method('DELETE')
                                <button type="submit" class="px-4 py-2 bg-rose-50 text-rose-600 border border-rose-200 rounded-xl text-xs font-bold active:scale-95 transition-transform">
                                    Delete
                                </button>
                            </form>
                        </div>
        ...
        <div x-show="addModal" class="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-sm" x-cloak>
            <div class="bg-white rounded-2xl sm:rounded-[2rem] p-6 sm:p-10 max-w-xl w-full shadow-2xl border border-white max-h-[90vh] flex flex-col" @click.away="addModal = false">
                <div class="mb-4 shrink-0">
                    ...
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Super Admin User Provisioning</p>
                </div>
                ...
                {{-- Super Admin Store Route --}}
                <form action="{{ route('super-admin.users.store') }}" method="POST" class="space-y-4 sm:space-y-5 overflow-y-auto pr-1">
                    @csrf
        ...
        <div x-show="editModal" class="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-sm" x-cloak>
            <div class="bg-white rounded-2xl sm:rounded-[2.5rem] p-6 sm:p-10 max-w-2xl w-full shadow-2xl border border-white max-h-[90vh] flex flex-col" @click.away="editModal = false">
                ...
                <form :action="'/super-admin/users/' + currentUser.id" method="POST" class="space-y-4 sm:space-y-6 overflow-y-auto pr-1">
                    @csrf @method('PATCH')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        <div class="space-y-1">
                            ...
                            <input type="text" name="name" x-model="currentUser.name"
                                class="w-full rounded-xl sm:rounded-2xl border-slate-200 bg-slate-50 text-sm py-3 sm:py-4 px-4 sm:px-5 focus:ring-[#D4AF37] focus:border-[#D4AF37]"
                                pattern="[a-zA-ZñÑ\s\.\-']+"
                                oninput="this.value = this.value.replace(/[^a-zA-ZñÑ\s\.\-']/g, '')"
                                required>
                        </div>
        ...
        <div x-show="massEnrollModal" class="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-sm" x-cloak>
            <div class="bg-white rounded-2xl sm:rounded-[2.5rem] p-6 sm:p-8 max-w-2xl w-full shadow-2xl border border-white max-h-[90vh] flex flex-col" @click.away="massEnrollModal = false">
                ...
                <div class="overflow-y-auto pr-1">
                    ...
                    <!-- File Upload Form -->
                    <form action="{{ route('super-admin.users.import') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
